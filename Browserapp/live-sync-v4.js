...
const { applyTrustedChange, applyTrustedInput } = require('./automation/trusted-input');
...
class LiveSyncController {
  ...
  async forward(tabId, payload) {
    ...
    if (payload.type === 'click') {
      ...
      await this.eachSlave(tabId, async (tab) => {
        const call = (method, params = {}) => cdp.call(tab.webSocketDebuggerUrl, method, params);
        let result = await applyTrustedInput(call, payload);
        const x = Number(payload.x) || 0;
        const y = Number(payload.y) || 0;
        if (!result.ok && x && y) {
          await call('Input.dispatchMouseEvent', { type: 'mousePressed', x, y, button: 'left', clickCount: 1 });
          await call('Input.dispatchMouseEvent', { type: 'mouseReleased', x, y, button: 'left', clickCount: 1 });
          result = await applyTrustedInput(call, { ...payload, selector: '' });
        }
        if (!result.ok && this.emit) this.emit({ type: 'sync-input-skipped', tabId, reason: result.reason || 'native-input-failed' });
      });
    } else if (payload.type === 'change') {
      await this.eachSlave(tabId, async (tab) => {
        const call = (method, params = {}) => cdp.call(tab.webSocketDebuggerUrl, method, params);
        const result = await applyTrustedChange(call, payload, { allowSyntheticFallback: true });
        if (!result.ok && this.emit) this.emit({ type: 'sync-change-skipped', tabId, reason: result.reason || 'native-change-failed' });
        else if (result.fallback && this.emit) this.emit({ type: 'sync-change-fallback', tabId, reason: result.reason || 'native-change-failed' });
      });
