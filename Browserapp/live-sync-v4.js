...
const injection = String.raw`(() => {
  ...
  document.addEventListener('change', (event) => { const target = actual(event); const isSelect = String(target?.tagName || '').toLowerCase() === 'select'; const value = 'value' in target ? target.value : ''; const selectedIndex = isSelect && Number.isInteger(target.selectedIndex) ? target.selectedIndex : -1; send('change', { ...focusState(target), value: String(value ?? ''), selectedIndex }); }, true);
...
class LiveSyncController {
  ...
  async forward(tabId, payload) {
    ...
    if (payload.type === 'click') {
      ...
      const selector = JSON.stringify(String(payload.selector || '')); const value = JSON.stringify(String(payload.value || '')); const selectedIndex = Number.isInteger(payload.selectedIndex) ? payload.selectedIndex : -1;
      const expression = `(() => { const deep=(path)=>{let root=document,e=null;for(const part of path.split(/\s*>>>\s*/)){e=root.querySelector(part);if(!e)return null;root=e.shadowRoot||e;}return e;}; const e=deep(${selector})||document.querySelector(${selector})||(document.activeElement?.tagName?.toLowerCase()==='select'?document.activeElement:null); if(!e)return false; if(e.tagName&&e.tagName.toLowerCase()==='select'){if(${selectedIndex}>=0&&${selectedIndex}<e.options.length){e.selectedIndex=${selectedIndex};}else if(${value}!==''){e.value=${value};}}else if('value' in e){e.value=${value};} e.dispatchEvent(new Event('input',{bubbles:true,composed:true})); e.dispatchEvent(new Event('change',{bubbles:true,composed:true})); return true; })()`;
      await this.eachSlave(tabId, (tab) => cdp.call(tab.webSocketDebuggerUrl, 'Runtime.evaluate', { expression }));
